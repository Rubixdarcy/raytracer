...
const CLOCK_CANVAS_RATIO: f64 = 0.8;
...
fn main() {
    ...
    let clock_scale: f64 = CANVAS_SIZE as f64 * CLOCK_CANVAS_RATIO / 2.0;
    let clock_scale = scaling(clock_scale, clock_scale, clock_scale);
    let clock_translation = translation(CANVAS_SIZE as f64 / 2.0, CANVAS_SIZE as f64 / 2.0, 0.0);
    let clock_transform = clock_translation * clock_scale;

    for i in 0..12i32 {
        let hour_transform = rotation_z((i as f64) * std::f64::consts::PI / 6.0);
